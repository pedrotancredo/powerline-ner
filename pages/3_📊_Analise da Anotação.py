...
st.markdown("# Analise da Anotação")
st.markdown("""Analise do arquivo de saída do Label Studio:""")
...
label_text_list = [
    (label_item["labels"][0], label_item["text"])
    for item in json_content
    for label_item in item.get("label", [])
    if label_item.get("labels")
]
...
selected_label = st.selectbox(
    "Escolha um rótulo:",
    list(set(labels_list)),
)

# Filter the label_text_list based on selected label
filtered_label_text_list = [
    (label, text) for label, text in label_text_list if label == selected_label
]
...
word_frequency_df = pd.DataFrame(
    word_frequency.items(), columns=["Texto", "Frequência"]
)
# Max lengh Texto
...
chart = (
    ...
    .properties(width=600, height=max_height)
)
